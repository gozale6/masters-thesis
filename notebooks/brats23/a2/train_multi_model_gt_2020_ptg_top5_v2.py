...
=======================
Field-wise multi-task rewrite of train_multi_model.py.

WHY THIS IS DIFFERENT
---------------------
The v1 eval showed three problems that no amount of retraining the *same* setup
would fix:

  1. Format compliance = 0.0000 for ALL models.
     Root cause: parse_report() fell through to RAW prose whenever <2 fields
     matched, so the training targets were a MIX of clean 4-line skeletons and
     free text. The model learned the average (prose), so the eval's exact-format
     check never fired.

  2. Hemisphere accuracy "regressed" under augmentation (0.47 -> 0.22).
     Root cause was largely a METRIC artifact: _detect_hemisphere scanned the
     WHOLE report and returned BILATERAL whenever both "left" and "right"
     appeared anywhere. Aug models write longer reports -> mention more regions
     -> trip BILATERAL spuriously. Here we score hemisphere on the LESION field
     only.

  3. necrosis / compression dropped most under aug.
     Root cause: the "region shuffle" did a GLOBAL random.shuffle that destroyed
     the most-active-first ordering those position-sensitive fields rely on.
     Fixed: banded shuffle (only reorders rows of equal activity), default OFF.

THE NEW DESIGN (divide-and-conquer, but ONE model)
--------------------------------------------------
Instead of one model emitting a full 4-line report (which it never formatted
correctly), each training CASE is expanded into 4 training EXAMPLES:

    prompt + "TASK: report the LESION AREA."        -> "<lesion value>"
    prompt + "TASK: report the EDEMA."              -> "<edema value>"
    prompt + "TASK: report the NECROSIS."           -> "<necrosis value>"
    prompt + "TASK: report VENTRICULAR COMPRESSION."-> "<compression value>"

Each target is a SINGLE clean line. At inference we query the model 4 times and
ASSEMBLE the 4-line report ourselves. Consequences:

  * Format compliance is 1.0 by construction (we build the skeleton).
  * Each field gets dedicated supervision (your divide-and-conquer goal) WITHOUT
    4 separate checkpoints, and weights are shared so necrosis still "sees" what
    lesion learned.
  * Parsing at eval is trivial -> every PerField metric stops being capped by
    format failures.

Two data diagnostics run at build time and PRINT TO THE LOG before any training:
  * canonical parse rate  (how many GT reports actually yield clean fields)
  * hemisphere agreement   (prompt-inferred hemisphere vs GT-report hemisphere)
If the parse rate is low, fix your GT extraction FIRST -- training can't beat
bad targets. If hemisphere agreement is low, hemisphere-swap augmentation is
built on a wrong correlation and should stay OFF.

Usage
-----
    python train_multi_model_v2.py                      # all models
    python train_multi_model_v2.py --model scifive-base
    python train_multi_model_v2.py --audit_only         # just print diagnostics
    python train_multi_model_v2.py --no-skip_done

Outputs
-------
  checkpoints_multi_v2/<model_key>/   LoRA + tokenizer + summary.json
  logs/multi_model_v2_<ts>.log
  logs/summary_v2_<ts>.json
"""
...
# ── Model registry ────────────────────────────────────────────────────────────
# p_shuffle defaults to 0 everywhere now (the v1 shuffle was harmful). Turn it on
# per-run via --p_shuffle if you want to test the FIXED banded version.
MODELS = {
...
# ── Field definitions ─────────────────────────────────────────────────────────
# Order is fixed and used both for the assembled report and the per-field loop.
FIELDS = ["lesion", "edema", "necrosis", "compression"]

# Natural-language task selector per field. A pretrained T5 responds far better
# to a meaningful instruction than to a raw sentinel token.
FIELD_TASK = {
...
# How the final 4-line report is assembled at inference time. This is the EXACT
# string your eval's format-compliance check should match against.
REPORT_FORMAT = (
    "Lesion area: {lesion}\n"
    "Edema: {edema}\n"
    "Necrosis: {necrosis}\n"
...
def _ordered_active(pivot, banded_shuffle=False):
    """Return active rows sorted most-active-first. If banded_shuffle, only
    reorder rows that share the same total activity (preserves salience)."""
    active = pivot[(pivot > 0).any(axis=1)].copy()
    active["_total"] = active.sum(axis=1)
    active = active.sort_values("_total", ascending=False)
    if banded_shuffle:
        # Shuffle WITHIN equal-total bands only — never across salience levels.
        new_index = []
...
def matrix_to_context(case_id, pivot, banded_shuffle=False):
    """Build ONLY the patient/matrix context (no task line, no field format)."""
...
def parse_fields(raw):
    """Return (fields_dict, n_matched). fields_dict has all 4 keys; unmatched
    are None so the caller can decide whether to keep the case."""
    text = clean_text(raw)
    # parse_fields needs newlines for the lookaheads; clean_text collapsed them.
    # Re-introduce a soft split on the field keywords so lookaheads still work.
...
def report_hemisphere(lesion_value):
    """Hemisphere from the LESION field only (the v1 bug was scanning the whole
    report). Used for the data diagnostic and the training quick-check."""
    if not lesion_value:
        return "UNK"
...
def build_cases(csv_path, text_dir, log, min_fields=2):
    """Build per-CASE records with canonical field dicts. Runs diagnostics and
    drops cases whose GT report yields fewer than `min_fields` parsed fields."""
    ...
    cases = []
    n_no_report = 0
    match_hist = Counter()           # how many fields parsed per case
    hemi_agree = hemi_total = 0      # prompt-inferred vs GT lesion hemisphere
    ...
    for case_id, case_df in df.groupby("case_id"):
        ...
        # Hemisphere agreement diagnostic
        ...
        # Fill any missing field with the canonical "not observed"
    ...
    kept = len(cases)
    log.info(f"  Cases KEPT (>= {min_fields} fields)            : {kept}")
    if total_with_report:
        full = match_hist.get(4, 0)
        log.info(f"  Cases parsing ALL 4 fields cleanly  : {full} "
                 f"({100*full/total_with_report:.1f}%)")
        if match_hist.get(0, 0) + match_hist.get(1, 0) > 0.3 * total_with_report:
            log.warning("  >>> Over 30% of reports parse <2 fields. Your GT "
                        "EXTRACTION is the bottleneck, not the model. Fix "
                        "_FIELD_PATTERNS / the TextBraTS text before training.")
    agree_pct = 100 * hemi_agree / hemi_total if hemi_total else 0
    log.info(f"  Hemisphere agreement (prompt vs GT) : {hemi_agree}/{hemi_total} "
             f"= {agree_pct:.1f}%")
    if hemi_total and agree_pct < 70:
        log.warning("  >>> Prompt-inferred hemisphere disagrees with GT a lot. "
                    "The model CANNOT learn to copy a field the prompt gets "
                    "wrong. Keep hemisphere-swap augmentation OFF until this is "
                    "fixed, and consider trusting the GT hemisphere in the prompt.")
    log.info("=" * 70 + "\n")
...
def expand_to_examples(cases):
    """One case -> 4 field examples. Returns list of dicts."""
    examples = []
    for c in cases:
        for field in FIELDS:
            examples.append({
...
class FieldDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and random.random() < self.p_swap:
            ...
            # Only laterality-bearing fields need their target swapped.
...
def predict_report(model, tokenizer, case, device, max_new_tokens, max_input):
    """Query the model once per field and ASSEMBLE the canonical 4-line report.
    This is what your eval should call instead of a single full-report generate.
    Format compliance is 1.0 by construction."""
    context = case["context"]
    values = {}
    for field in FIELDS:
        prompt = build_field_prompt(context, field)
...
def quick_hemisphere_check(model, tokenizer, val_cases, device,
                           max_new, max_input, log, n=20):
    """Hemisphere accuracy on the LESION field only (fixes the v1 metric bug)."""
    model.eval()
    chosen = random.sample(val_cases, min(n, len(val_cases)))
    correct = total = 0
    for c in chosen:
...
def train_one_model(model_key, args, cases, log):
    ...
    # Split on CASES (not examples) so all 4 fields of a case stay on one side —
    # prevents leakage between train and val.
    train_cases, val_cases = train_test_split(
        cases, test_size=0.20, random_state=args.seed, shuffle=True,
    )
    ...
    lora_cfg = LoraConfig(
        task_type=TaskType.SEQ_2_SEQ_LM, r=32, lora_alpha=64,
        target_modules=cfg["target_modules"], lora_dropout=0.1, bias="none",
    )
    try:
        model = get_peft_model(model, lora_cfg)
    except ValueError as e:
        log.warning(f"LoRA target {cfg['target_modules']} failed: {e}; auto-detecting.")
...
def parse_args():
    ...
    p.add_argument("--output_root", default="checkpoints_multi_v2")
    ...
    p.add_argument("--min_fields",    type=int,   default=2,
                   help="Drop GT reports parsing fewer than this many fields")
    p.add_argument("--p_swap",        type=float, default=0.5,
                   help="Hemisphere-swap prob (aug models only). Set 0 if "
                        "hemisphere agreement diagnostic is low.")
    p.add_argument("--p_shuffle",     type=float, default=0.0,
                   help="FIXED banded region-shuffle prob (aug models only). "
                        "Default 0 — the v1 global shuffle hurt; enable only to test.")
    ...
    p.add_argument("--audit_only",    action="store_true",
                   help="Run data diagnostics and exit (no training)")
...
def main():
    ...
    log_path = Path("logs") / f"multi_model_v2_{ts}.log"
    ...
    out_path = Path("logs") / f"summary_v2_{ts}.json"
