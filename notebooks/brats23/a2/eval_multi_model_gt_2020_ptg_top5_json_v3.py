...
# (v3) Front-loaded, distinct questions — MUST match the v3 trainer verbatim.
FIELD_TASK = {
...
# (v3) Trailing answer cue that bookends the field name at the END of the prompt.
FIELD_LABEL = {
    "lesion":      "LESION AREA",
    "edema":       "EDEMA",
    "necrosis":    "NECROSIS",
...
# (v3) Short matrix header — the long system preamble was removed in v3.
...
TOP_N_REGIONS = 5   # must match train_multi_model_v3_json.py


def matrix_to_context(case_id, pivot):
    """Valerio-style JSON descriptor — MUST produce byte-identical output to the
    JSON trainer's matrix_to_context (banded_shuffle=False path)."""
    ...
    spatial = [
        {"Region": region_name, "Percentage_of_Tumor": round(val / grand * 100, 2)}
...
def build_field_prompt(context, field):
    """(v3) Front-load the field question; bookend with a field-named answer cue."""
    return (
        f"{FIELD_TASK[field]}\n"
...
def parse_args():
    ...
    p.add_argument("--checkpoint_root", default="checkpoints_multi_v3_json")
    p.add_argument("--csv",             default="atlas_segmentations/ground_truth/all_cases.csv")
    p.add_argument("--text_dir",        default="../TextBraTSData")
    p.add_argument("--output_dir",      default="eval_multi_results_v3_json")
...
def main():
    ...
    log_path = Path("logs") / f"eval_multi_v3_json_{ts}.log"
    ...
    log.info("=" * 70)
    log.info("Field-wise multi-model evaluation (v3-JSON, Valerio-style input)")
    log.info("=" * 70)
