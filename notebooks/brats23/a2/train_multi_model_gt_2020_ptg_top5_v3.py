...
def matrix_to_context(case_id, pivot, banded_shuffle=False):
    """Build ONLY the matrix body (no instruction, no field). The instruction
    now lives in build_field_prompt so it can be front-loaded."""
...
def parse_args():
    ...
    p.add_argument("--output_root", default="checkpoints_multi_v3")
...
def main():
    ...
    log_path = Path("logs") / f"multi_model_v3_{ts}.log"
    logging.basicConfig(level=logging.INFO, format="%(asctime)s  %(levelname)s  %(message)s",
                        handlers=[logging.FileHandler(log_path), logging.StreamHandler()], force=True)
    log = logging.getLogger(__name__)

    log.info("=" * 70)
    log.info("Field-wise multi-task BraTS matrix → report training (v3, anti-collapse)")
    log.info("=" * 70)
    ...
    out_path = Path("logs") / f"summary_v3_{ts}.json"
