...
====================
Trains three candidate models on the BraTS matrix → structured report task
using GROUND-TRUTH atlas matrices (~369 BraTS20 cases, not only predictions).

Models
------
  1. razent/SciFive-base-PMC           (seq2seq T5-base, PMC-pretrained)
  2. razent/SciFive-large-Pubmed       (seq2seq T5-large, PubMed-pretrained)
  3. hossboll/clinical-t5              (seq2seq T5, clinical summarization)

Usage
-----
    python train_multi_model.py
    python train_multi_model.py --model scifive-large
    python train_multi_model.py --csv atlas_segmentations/ground_truth/all_cases.csv

Outputs
-------
  checkpoints_multi/
    scifive-base/       <LoRA files + summary.json>
    scifive-large/...
    clinicalt5-hoss/...
  logs/
...
def preview_generations(model, tokenizer, val_samples, device, max_new, max_input, log, n=3):
    model.eval()
...
def train_one_model(model_key, args, samples, log):
    ...
    train_ds = Seq2SeqDataset(train_samples, tokenizer, cfg["max_input"], cfg["max_target"])
    val_ds   = Seq2SeqDataset(val_samples,   tokenizer, cfg["max_input"], cfg["max_target"])
    ...
    # Large model got a worse val loss with 3e-5 in the previous run; use a
    # more conservative LR for it.
...
def parse_args():
    ...
    p.add_argument("--epochs",        type=int,   default=500)    # 5x more data → fewer epochs needed
    p.add_argument("--batch_size",    type=int,   default=2)
    p.add_argument("--grad_accum",    type=int,   default=4)
    p.add_argument("--lr",            type=float, default=3e-5)   # base models
    p.add_argument("--lr_large",      type=float, default=1e-5)   # large model (more conservative)
...
def main():
    ...
    log.info("=" * 70)
    log.info("Multi-model BraTS matrix → report training (ground truth)")
    log.info("=" * 70)
