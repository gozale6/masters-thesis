...
class Config:
    ...
    # Output
    save_preds    = True        # set True to write NIfTI predictions
    output_dir    = "./predictions_segresnet_2020"
...
def run_inference(checkpoint_path, data_json, save_preds):
    print("=" * 70)
    print("SegResNet Inference — BraTS 2023")
    print("=" * 70)
    ...
    # Extract patient IDs from image paths (parent folder name)
    patient_ids = [
        os.path.basename(os.path.dirname(entry["image"][0]))
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(loader, desc="Inference")):
            ...
            if save_preds:
                ...
                nib.save(img, os.path.join(config.output_dir, f"pred_{patient_ids[idx]}.nii.gz"))
    ...
    # Per-case summary
    print("\nPer-case breakdown:")
    print(f"  {'Patient ID':30}  {'ET':>6}  {'TC':>6}  {'WT':>6}  {'Avg':>6}  "
          f"{'HD95_ET':>8}  {'HD95_TC':>8}  {'HD95_WT':>8}  {'HD95_Avg':>9}")
    print("  " + "-" * 95)
    for c in per_case:
        print(f"  {c['patient_id']:30}  {c['et']:>6.4f}  {c['tc']:>6.4f}  {c['wt']:>6.4f}  {c['avg']:>6.4f}  "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SegResNet inference on BraTS 2023")
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="./checkpoints_segresnet/best_model.pth",
