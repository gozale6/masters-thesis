...
FINE-TUNE-MODE AXIS (matches the 12-run trainer)
    The trainer now writes one checkpoint dir PER (model, ft_mode):
        <output_root>/<model_key>-lora/   (base model + LoRA adapter)
        <output_root>/<model_key>-full/   (a FULL AutoModelForSeq2SeqLM save)
    This script mirrors that: --ft_mode {lora|full|both} (default both) and it
    LOADS EACH DIR THE RIGHT WAY — it auto-detects via adapter_config.json:
        * adapter present  -> base model + PeftModel.from_pretrained(adapter)
        * no adapter (full)-> AutoModelForSeq2SeqLM.from_pretrained(dir) directly
    predictions_<model_key>-<ft_mode>.json keeps the two apart.
...
What it does
    ...
    3. Loads each checkpoint (LoRA adapter OR full model, per above).
    ...
    5. Computes metrics (incl. the v4 cross-patient template-collapse ratio) and
       writes predictions_<run_key>.json in the same schema you've been using.
...
Usage
-----
    python eval_multi_model_gt_2020_ptg_v4.py                          # all models, lora+full
    python eval_multi_model_gt_2020_ptg_v4.py --ft_mode lora           # LoRA checkpoints only
    python eval_multi_model_gt_2020_ptg_v4.py --ft_mode full           # full-FT checkpoints only
    python eval_multi_model_gt_2020_ptg_v4.py --model scifive-base     # both modes for one model
...
FT_MODES          = getattr(tm, "FT_MODES", ["lora", "full"])
...
def _is_lora_dir(out_dir):
    return (out_dir / "adapter_config.json").exists() or (out_dir / "adapter_model.safetensors").exists()


def _is_full_dir(out_dir):
...
def load_checkpoint(model_key, ft_mode, out_dir, log):
    """Load a checkpoint the right way for its ft_mode.

    LoRA  : base model (from summary/registry) + PeftModel.from_pretrained(adapter).
    full  : AutoModelForSeq2SeqLM.from_pretrained(out_dir) directly (no adapter).

    ft_mode is taken from the run key, but the actual load path is decided by
    what's ON DISK (adapter_config.json present or not), so a mislabeled dir
    still loads correctly. Returns (model, tokenizer, cfg_like).
    """
    ...
    if summary_path.exists():
        ...
        summary_ft_mode = s.get("ft_mode", ft_mode)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    tokenizer = AutoTokenizer.from_pretrained(str(out_dir), use_fast=True)

    on_disk_lora = _is_lora_dir(out_dir)
    if on_disk_lora and ft_mode == "full":
        log.warning(f"[{model_key}-{ft_mode}] dir has a LoRA adapter but ft_mode=full — "
                    "loading as LoRA (trusting disk).")
    if (not on_disk_lora) and ft_mode == "lora":
        log.warning(f"[{model_key}-{ft_mode}] dir has no adapter but ft_mode=lora — "
                    "loading as a full model (trusting disk).")
    ...
    if on_disk_lora:
        ...
        # full fine-tune: the whole model was saved into out_dir
        model = AutoModelForSeq2SeqLM.from_pretrained(str(out_dir), torch_dtype=torch.float32)
        load_kind = "full"

    # training may have persisted use_cache=False (grad checkpointing); generation
    # wants it on.
    try:
    ...
    log.info(f"  loaded {load_kind} checkpoint (base={base_name}, summary_ft_mode={summary_ft_mode})")
...
def evaluate_one(model_key, ft_mode, args, cases, log):
    run_key = f"{model_key}-{ft_mode}"
    out_dir = Path(args.output_root) / run_key
    if not out_dir.exists() or not (_is_lora_dir(out_dir) or _is_full_dir(out_dir)):
        log.warning(f"[{run_key}] no usable checkpoint in {out_dir} — skipping.")
        return {"model_key": model_key, "ft_mode": ft_mode, "run_key": run_key,
                "error": "no checkpoint"}

    log.info("\n" + "#" * 70)
    log.info(f"# Evaluating {run_key}  ({out_dir})")
    log.info("#" * 70)

    model, tokenizer, cfg = load_checkpoint(model_key, ft_mode, out_dir, log)
    if cfg["lobe_rollup"] != args.lobe_rollup:
        log.warning(f"[{run_key}] checkpoint trained with lobe_rollup={cfg['lobe_rollup']} but eval "
    ...
    # predictions JSON (your existing schema) — run_key keeps lora/full separate
    pred_path = Path(args.results_dir) / f"predictions_{run_key}.json"
    ...
    return {"model_key": model_key, "ft_mode": ft_mode, "run_key": run_key,
            "model_name": cfg["model_name"], "load_kind": cfg["load_kind"],
...
def parse_args():
    ...
    p.add_argument("--ft_mode",     choices=["lora", "full", "both"], default="both",
                   help="Which fine-tune mode(s) to evaluate. 'both' evals every "
                        "<model>-lora and <model>-full dir found -> up to 12.")
...
def main():
    ...
    for i, (key, mode) in enumerate(runs, 1):
        log.info(f"\n>>> Eval {i}/{len(runs)}  [{key}-{mode}]")
        try:
            results.append(evaluate_one(key, mode, args, cases, log))
        except Exception as e:
            log.exception(f"[{key}-{mode}] eval FAILED: {e}")
            results.append({"model_key": key, "ft_mode": mode,
                            "run_key": f"{key}-{mode}", "error": str(e)})
    ...
    for i, r in enumerate(rank, 1):
        ...
        mode = r.get("ft_mode", "lora")
        log.info(f"  {i}. {r['model_key']:<22}{tag} [{mode:<4}] rougeL={m.get('rougeL', 0):.4f}  "
    ...
    # head-to-head: LoRA vs full per base model (what you added the axis for)
    by_model = {}
    for r in results:
        if "metrics" in r:
            by_model.setdefault(r["model_key"], {})[r.get("ft_mode", "lora")] = r["metrics"]
    pairs = [(k, v) for k, v in by_model.items() if "lora" in v and "full" in v]
    if pairs:
        log.info("\n" + "=" * 70)
        log.info("LoRA vs FULL fine-tuning (delta = full - lora)")
        log.info("=" * 70)
        for k, v in sorted(pairs):
            for metric in ("rougeL", "hemisphere_accuracy", "lobe_jaccard", "cross_patient_lesion"):
                lo = v["lora"].get(metric, 0.0) or 0.0
                fu = v["full"].get(metric, 0.0) or 0.0
                log.info(f"  {k:<22} {metric:<20} lora={lo:.4f}  full={fu:.4f}  Δ={fu-lo:+.4f}")

    failed = [r for r in results if "error" in r]
    if failed:
        log.info("\n  FAILED/SKIPPED:")
        for r in failed:
            log.info(f"    {r.get('run_key', r['model_key'])}: {r['error']}")
